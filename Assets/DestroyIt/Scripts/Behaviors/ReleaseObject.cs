...
namespace DestroyIt
{
    public class ReleaseObject : MonoBehaviour
    {
        ...
        void Start()
        {
            _velocityLastUpdate = GetComponent<Rigidbody>().linearVelocity;
        }

        void FixedUpdate()
        {
            Vector3 velocityChange = (GetComponent<Rigidbody>().linearVelocity - _velocityLastUpdate) / GetComponent<Rigidbody>().mass;
